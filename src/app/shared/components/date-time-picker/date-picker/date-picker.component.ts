...
export class DatePickerComponent implements OnInit {
  ...
  constructor(private renderer: Renderer2) {
    this.renderer.listen('window', 'click',(e:Event)=>{
      ...
      if(!this.toggleButton.nativeElement.contains(e.target) && !this.previous.nativeElement.contains(e.target) && !this.after.nativeElement.contains(e.target) && !this.control.nativeElement.contains(e.target)
      )  {
          this.showed=false;
      }
    });
  ...
  getDayOfMonth(month) {
    ...
    if (week.length > 0) this.listWeek.push(week);
  }
